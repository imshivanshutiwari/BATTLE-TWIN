...
class SystemEvaluator:
    ...
    def _create_test_state(self) -> BattlefieldState:
        ...
        for i in range(12):
            state.add_unit(UnitState(
                uid=f"B{i:02d}", callsign=f"WARHORSE-{i+1}", unit_type="infantry",
                lat=34.05 + i*0.02, lon=-117.45 + i*0.01,
                ammo_pct=80-i*3, fuel_pct=90-i*2, water_pct=85-i*2,
            ))
        for i in range(5):
            state.add_contact(ContactReport(
                uid=f"R{i:02d}", callsign=f"HOSTILE-{i+1}",
                lat=34.30 + i*0.02, lon=-117.15 + i*0.01, confidence=0.7+i*0.05,
            ))
        return state
    ...
    def evaluate_sim_performance(self, n_ticks: int = 100) -> Dict:
        ...
        return {"ticks": n_ticks, "elapsed_s": elapsed,
    ...
    def evaluate_agent_performance(self) -> Dict:
        ...
        return {"decision_time_ms": elapsed_ms,
                "agents_ran": list(result.get("agent_outputs", {}).keys()) if "agent_outputs" in result else ["s2","s3","fso","s4","css"],
    ...
    def evaluate_sync_accuracy(self) -> Dict:
        ...
        return {"mean_error_m": float(np.mean(errors)),
