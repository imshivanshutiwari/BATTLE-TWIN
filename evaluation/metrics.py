...
class C2Metrics:
    ...
    def c2_latency_stats(self) -> Dict:
        ...
        return {"mean": float(np.mean(arr)), "p50": float(np.percentile(arr, 50)),
                ...
                "max": float(np.max(arr)), "count": len(arr)}
    ...
    def sync_accuracy_stats(self) -> Dict:
        ...
        return {"mean_error_m": float(np.mean(arr)), "max_error_m": float(np.max(arr)),
    ...
    def decision_time_stats(self) -> Dict:
        ...
        return {"mean_ms": float(np.mean(arr)), "max_ms": float(np.max(arr)),
                "under_500ms_pct": float((arr < 500).sum() / len(arr) * 100)}

    def throughput_stats(self) -> Dict:
        return {"total_messages": sum(self._message_counts.values()),
                "by_topic": dict(self._message_counts)}

    def all_metrics(self) -> Dict:
        return {"latency": self.c2_latency_stats(), "sync": self.sync_accuracy_stats(),
                "decisions": self.decision_time_stats(), "throughput": self.throughput_stats()}
