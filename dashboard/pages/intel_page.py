...
def create_intel_map():
    ...
    fig.update_layout(**PLOTLY_TEMPLATE, mapbox_style="carto-darkmatter",
                      ...
                      height=350, title="INTEL OVERLAY — CONTACTS")
...
def create_threat_heatmap():
    ...
    fig = go.Figure(go.Heatmap(z=z, colorscale=[[0,"#003300"],[0.5,"#ffcc00"],[1,"#ff3333"]],
                                showscale=True, colorbar=dict(title="Threat")))
...
def create_contact_timeline():
    ...
    for i, c in enumerate(contacts):
        fig.add_trace(go.Bar(y=[c], x=[3+i*0.5], orientation='h', name=c,
                             marker_color=NVG_THEME["hostile"]))
    fig.update_layout(**PLOTLY_TEMPLATE, height=200, title="CONTACT TIMELINE (6H)",
                      xaxis_title="Hours", barmode="stack")
...
def create_pattern_of_life():
    ...
    fig.update_layout(**PLOTLY_TEMPLATE, mapbox_style="carto-darkmatter",
                      ...
                      height=250, title="PATTERN OF LIFE — ADS-B")
