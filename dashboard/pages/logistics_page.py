...
def create_supply_dashboard():
    ...
    fig.update_layout(
        **PLOTLY_TEMPLATE, height=250, title="SUPPLY STATUS BY CLASS", barmode="group"
    )
...
def create_vrp_viz():
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="lines+markers",
            name="Time (min)",
            line=dict(color=NVG_THEME["friendly"]),
            yaxis="y2",
        )
    )
    fig.update_layout(
