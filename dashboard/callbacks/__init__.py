...
def register_callbacks(app):
    ...
    def update_unit_table(_):
        ...
        for uid, u in _battlefield_state.units.items():
            color = NVG_COLORS["text"] if u.strength_pct > 50 else NVG_COLORS["yellow"] if u.strength_pct > 25 else NVG_COLORS["red"]
            rows.append(html.Tr([
                html.Td(u.callsign, style={"color": color, "fontSize": "9px"}),
                html.Td(f"{u.strength_pct:.0f}%", style={"color": color, "fontSize": "9px"}),
                html.Td(u.comms_status, style={"fontSize": "9px"}),
            ]))
        return html.Table(
            [html.Tr([html.Th("CALL", style={"fontSize": "9px"}), html.Th("STR", style={"fontSize": "9px"}), html.Th("COMMS", style={"fontSize": "9px"})])] + rows,
    ...
    def update_tac_map(_):
        ...
        if _battlefield_state:
            ...
            fig.add_trace(go.Scattermapbox(lat=f_lats, lon=f_lons, text=f_names,
                marker=dict(size=12, color=NVG_COLORS["blue"]), mode="markers+text",
                name="FRIENDLY", textposition="top right"))
            ...
            fig.add_trace(go.Scattermapbox(lat=h_lats, lon=h_lons, text=h_names,
                marker=dict(size=10, color=NVG_COLORS["red"]), mode="markers+text",
                name="HOSTILE", textposition="top right"))
            # Center map
    ...
    def update_threat_gauge(_):
        ...
        fig.add_trace(go.Indicator(
            ...
            gauge=dict(axis=dict(range=[0, 100]),
                       bar=dict(color=NVG_COLORS["red"] if threat > 0.7 else NVG_COLORS["yellow"] if threat > 0.4 else NVG_COLORS["text"]),
    ...
    def update_alerts(_):
        ...
        for a in reversed(alerts):
            color = NVG_COLORS["red"] if a.get("level") in ("FLASH", "IMMEDIATE") else NVG_COLORS["yellow"]
            items.append(html.Div(f"[{a.get('level', '')}] {a.get('details', '')}", style={"color": color, "fontSize": "9px", "marginBottom": "2px"}))
    ...
    def update_sync(_):
        ...
        if _battlefield_state:
            return html.Div([
                html.Div(f"SEQ: {_battlefield_state.nats_sequence}", style={"fontSize": "10px"}),
                html.Div(f"UNITS: {len(_battlefield_state.units)}", style={"fontSize": "10px"}),
                html.Div(f"CONTACTS: {len(_battlefield_state.contacts)}", style={"fontSize": "10px"}),
                html.Div("STATUS: SYNCED", style={"color": NVG_COLORS["accent"], "fontSize": "10px"}),
            ])
