...
class DEMProcessor:
    ...
    def compute_hillshade(self, dem: np.ndarray, azimuth_deg: float = 315.0,
    ...
    def get_statistics(self, dem: np.ndarray) -> Dict:
        return {
            "min_elev": float(np.min(dem)), "max_elev": float(np.max(dem)),
            "mean_elev": float(np.mean(dem)), "std_elev": float(np.std(dem)),
            "shape": dem.shape,
        }
