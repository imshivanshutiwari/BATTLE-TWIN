...
class FSOFiresAgent:
    ...
    def _rule_based_fires(self, targets, friendlies) -> Dict:
        ...
        for i, t in enumerate(targets):
            ...
            min_dist = float('inf')
            for f in friendlies:
                dist = ((t_lat - f.get("lat", 0))**2 + (t_lon - f.get("lon", 0))**2)**0.5 * 111320
                min_dist = min(min_dist, dist)
            is_danger_close = min_dist < 600
            missions.append({
                "mission_id": f"FM-{i+1:03d}", "target": t,
                "method": "ADJUST_FIRE" if is_danger_close else "FIRE_FOR_EFFECT",
                "ammunition": "HE", "danger_close": is_danger_close,
    ...
    def call_for_fire(self, observer: str, target_lat: float, target_lon: float,
                      target_desc: str) -> str:
        return (f"CFF (Call For Fire)\n"
                f"OBSERVER: {observer}\n"
