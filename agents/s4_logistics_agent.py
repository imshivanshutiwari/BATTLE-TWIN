        ...
        if self._llm:
            try:
                resp = self._llm.invoke(
                    [
                        SystemMessage(content=S4_PROMPT),
                        HumanMessage(
                            content=f"Assess logistics: {json.dumps(units[:10], default=str)}"
                        ),
                    ]
                )
                try:
                ...
                critical.append(
                    {
                        ...
                        "critical_type": (
                            "ammo"
                            if ammo == min_supply
                            else "fuel" if fuel == min_supply else "water"
                        ),
                        "level": min_supply,
                    }
                )
        ...
        return {
            ...
            "supply_status": (
                ...
                if critical
                    ...
                    "AMBER"
                    if priority_list and priority_list[0]["priority_score"] > 30
                    else "GREEN"
                )
            ),
        }
...
if __name__ == "__main__":
    ...
    units = [
        {"uid": "B01", "ammo_pct": 25, "fuel_pct": 40, "water_pct": 60},
