...
class S3ManeuverAgent:
    ...
    def plan_maneuver(
        self, units: List[Dict], objectives: List[Dict], terrain: Dict = None, threats: Dict = None
    ) -> Dict[str, Any]:
        prompt = (
            f"Plan maneuver for units:\n{json.dumps(units[:10], default=str)}\n"
            f"Objectives: {json.dumps(objectives, default=str)}"
        )
        if self._llm:
            try:
                resp = self._llm.invoke(
                    [SystemMessage(content=S3_SYSTEM_PROMPT), HumanMessage(content=prompt)]
                )
                try:
    ...
    def _rule_based_plan(self, units, objectives) -> Dict:
        ...
        return {
            "orders": [
                {"unit": a, "objective": o, "action": "ATTACK"} for o, a in assignments.items()
            ],
            "movement_plan": {
                "formation": "WEDGE",
                "speed": "DELIBERATE",
                "route": "COVERED_CONCEALED",
            },
...
if __name__ == "__main__":
    ...
    units = [
        {"callsign": "WARHORSE-1", "type": "infantry"},
        {"callsign": "WARHORSE-2", "type": "armor"},
    ]
