    ...
    def _move_units(self, state: BattlefieldState):
        for uid, unit in state.units.items():
            if unit.speed_mps > 0:
                ...
                dlon = unit.speed_mps * math.sin(hdg_rad) * self.dt / (111320 * math.cos(math.radians(unit.lat)))
                unit.lat += dlat
                unit.lon += dlon
    ...
    def _update_contacts(self, state: BattlefieldState):
        for cid, contact in state.contacts.items():
            if contact.speed_mps > 0:
                ...
                contact.lon += contact.speed_mps * math.sin(hdg_rad) * self.dt / (111320 * max(0.01, math.cos(math.radians(contact.lat))))
    ...
    def _consume_supplies(self, state: BattlefieldState):
        for uid, unit in state.units.items():
            ...
            if unit.fuel_pct < 20 or unit.ammo_pct < 20 or unit.water_pct < 20:
                supply_type = "FUEL" if unit.fuel_pct < 20 else "AMMO" if unit.ammo_pct < 20 else "WATER"
                state.add_alert("PRIORITY", uid, "SUPPLY_LOW", f"{unit.callsign} {supply_type} critical")
    ...
    def _check_proximity_alerts(self, state: BattlefieldState):
        for uid, unit in state.units.items():
            for cid, contact in state.contacts.items():
                ...
                if dist < 2000:
                    state.add_alert("FLASH", uid, "PROXIMITY", f"{unit.callsign} within 2km of {contact.callsign}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        state.add_unit(UnitState(uid="B01", callsign="WARHORSE-1", unit_type="infantry",
                                 lat=34.05, lon=-117.45, speed_mps=2.0, heading_deg=45))
        state.add_contact(ContactReport(uid="R01", callsign="HOSTILE-1",
                                         lat=34.30, lon=-117.15))
